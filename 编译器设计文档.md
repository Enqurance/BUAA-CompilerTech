...
​		具体的实现可能和此处的设计稍微有一些出入，但不会特别大。sym考虑使用上一次的Symbol实现。

### 中间代码生成器

​		建立一个Generator类，用于描述一个生成器的对象，它可以调用符号表TableMaster，也可以调用语法树TreeHead。生成器通过递归下降地分析语法树的语法成分和操作符号表来实现中间代码生成的基本逻辑，即可以理解为在原有的语法树基础上插入新的**动作**来实现分析。
...
​		在类的内部设置一些标识码和缺省值，以应对同一类下不同的生成要求。
